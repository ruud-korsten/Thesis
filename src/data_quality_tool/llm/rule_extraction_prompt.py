...
def build_rule_extraction_prompt(domain_text: str, df_columns: list[str]) -> str:
    column_list_str = ", ".join(df_columns)

    logger.debug(
        "Building rule extraction prompt with %d columns and domain text length %d",
        len(df_columns),
        len(domain_text),
    )

    return f"""
    You are a data quality assistant. Your task is to extract structured validation rules and separate natural-language insights from the domain knowledge provided below.
