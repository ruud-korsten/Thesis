...
class RuleExecutor:
    def __init__(self, df: pd.DataFrame):
        ...
        # Dispatcher mapping rule type to handler methods
    ...
    def _handle_range(self, rule: Dict) -> pd.Series:
        ...
        for col in columns:
            ...
            series = self.df[col]
            if "min" in condition:
                violation_mask |= series < condition["min"]
            if "max" in condition:
                violation_mask |= series > condition["max"]
    ...
    def _handle_conditional(self, rule: Dict) -> pd.Series:
        ...
        if "column" in then_cond:
            ...
            # Support compact form: { "Weekly_Sales": { "min": 0.01 } }
    ...
    def apply_rules(self, rules: List[Dict]) -> pd.DataFrame:
        ...
        for rule in rules:
            ...
            try:
                ...
                # Fallback: capture message as note
